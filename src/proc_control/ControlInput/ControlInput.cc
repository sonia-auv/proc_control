...
namespace proc_control{
    ...
    ControlInput::~ControlInput(){ navigationOdomSubscriber_.shutdown(); }
    ...
    void ControlInput::OdometryCallback(const nav_msgs::Odometry::ConstPtr &odomIn) {
        ...
        positionTranslation_[2] = odomIn->pose.pose.position.x;
        positionOrientation_[0] = odomIn->pose.pose.orientation.x;
        positionOrientation_[1] = odomIn->pose.pose.orientation.y;
        positionOrientation_[2] = odomIn->pose.pose.orientation.z;

        velocityTranslation_[0] = odomIn->twist.twist.linear.x;
        velocityTranslation_[1] = odomIn->twist.twist.linear.y;
        velocityTranslation_[2] = odomIn->twist.twist.linear.z;
